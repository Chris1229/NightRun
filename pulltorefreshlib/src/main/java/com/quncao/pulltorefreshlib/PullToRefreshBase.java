 ...
 *******************************************************************************/
package com.quncao.pulltorefreshlib;
...
import com.quncao.pulltorefreshlib.internal.FlipLoadingLayout;
import com.quncao.pulltorefreshlib.internal.LoadingLayout;
import com.quncao.pulltorefreshlib.internal.RotateLoadingLayout;
import com.quncao.pulltorefreshlib.internal.Utils;
import com.quncao.pulltorefreshlib.internal.ViewCompat;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    public static final int SMOOTH_SCROLL_LONG_DURATION_MS = 510;
    ...
    protected int getPullToRefreshScrollDuration() {
        if (LoadingLayout.USE_LARK_STYLE && Mode.PULL_FROM_START == mCurrentMode)
            ...
            if (mShowViewWhileRefreshing) {
                ...
                switch (mCurrentMode) {
                    ...
                    case PULL_FROM_START:
                        ...
                        mHeaderLayout.switch2LoadingView();
                        break;
                }
            } else {
                smoothScrollTo(0);
    ...
    protected void onReset() {
        ...
        mHeaderLayout.switch2PullingView();

        smoothScrollTo(0);

        if (LoadingLayout.CUSTOM && (Mode.PULL_FROM_START == mCurrentMode)) {
            SimpleDateFormat format = new SimpleDateFormat("MM-dd HH:mm");
            getLoadingLayoutProxy().setLastUpdatedLabel("上次更新时间: " + format.format(new Date()));
        }
    }
    ...
    private void onScroll(int move) {
        if (!LoadingLayout.USE_LARK_STYLE)
            return;

        if (Mode.PULL_FROM_START == mCurrentMode) {
            if (move < 0) {
                move *= -1;
            } else {
                move = 0;
            ...
            int validMove = move - mHeaderLayout.getOriginalHeight();
            int loadingRootHeight = validMove >= 0 ? move : mHeaderLayout.getOriginalHeight();

            View loadingRoot = mHeaderLayout.getLarkLoadingRootView();
            ViewGroup.LayoutParams layoutParams = loadingRoot.getLayoutParams();
            layoutParams.height = loadingRootHeight;
            loadingRoot.requestLayout();
//			mHeaderLayout.getSlogan().setAlpha(validMove / (float)(mHeaderLayout.getSloganHeight()));
        } else {
    ...
    private void pullEvent() {
        ...
        if (newScrollValue != 0 && !isRefreshing()) {
            ...
            switch (mCurrentMode) {
                case PULL_FROM_END:
                    mFooterLayout.onPull(mCurrentMode, scale);
                    break;
                case PULL_FROM_START:
                default:
                    mHeaderLayout.onPull(mCurrentMode, scale);
                    break;
            }
    ...
    private class MyInterpolator implements Interpolator {
        ...
        public float getInterpolation(float t) {
            if (LoadingLayout.USE_LARK_STYLE && Mode.PULL_FROM_START == mCurrentMode) {
                if (null == va) {
                    va = ValueAnimator.ofFloat(0, 1f, 0.85f, 1f, 0.92f, 1f);
                    va.setDuration(getPullToRefreshScrollDuration() - 10);
