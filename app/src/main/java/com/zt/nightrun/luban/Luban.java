...
import static com.zt.nightrun.luban.Preconditions.checkNotNull;
...
public class Luban {
    ...
    public static Luban newInstance(Context context){
        return new Luban(Luban.getPhotoCacheDir(context));
    }
    ...
    private File thirdCompress(@NonNull File file) {
        String thumb = mCacheDir.getAbsolutePath() + File.separator + (TextUtils.isEmpty(filename) ? System.nanoTime() : filename);
        ...
        return compress(filePath, thumb, thumbW, thumbH, angle, (long) size * 2);
    }
    ...
    private File firstCompress(@NonNull File file) {
        ...
        String filePath = file.getAbsolutePath();
        String thumbFilePath = mCacheDir.getAbsolutePath() + File.separator + (TextUtils.isEmpty(filename) ? System.nanoTime() : filename) ;
